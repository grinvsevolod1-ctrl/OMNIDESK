...
function helpReply(): AssistantResult {
  return {
    reply:
      'Я управляю всей панелью: сводка и аналитика, менеджеры (создать/заблокировать/диалоги), каналы и прокси, переписки клиентов (показать, ответить, передать другому менеджеру), директивы и база знаний ИИ, расписания, серверы и деплой приложений. Опасные действия всегда прошу подтвердить. Скажите, что нужно — например «покажи диалоги без ответа», «что с серверами» или «создай менеджера Ивана».',
    actions: [],
    openSection: null,
    views: [],
    pending: null,
