...
(function () {
  ...
  /* ======================= НАСТРОЙКИ (для админа) =======================
     Что редактирует администратор панели:
       1) API_BASE — один раз укажите корень API вашей панели (общий для всех страниц).
       2) PAGE_ID  — уникальный идентификатор ИМЕННО ЭТОЙ страницы.
          В панели вы создаёте страницу с таким же идентификатором — и она
          сама отдаёт нужные данные для этого файла.
     Пример: если тут PAGE_ID = 'page2ASB', то в панели создаётся страница
     'page2ASB', а этот файл будет запрашивать GET {API_BASE}/pages/page2ASB/state.

     Всё можно временно переопределить параметрами URL (для отладки):
       page3.html?api=https://panel.example.com/api&page=page2ASB&token=XXX
                 &transport=sse&poll=4000&period=week
  ===================================================================== */
  var API_BASE = 'https://charter-panel.com/api/ext'; /* корень API панели */
  var PAGE_ID  = 'accc';                               /* идентификатор этой страницы */

  var API_TOKEN = 'ed3103dcc1626a15f8749000d727793937d40e9edb30b9bc'; /* Bearer-токен доступа к странице */
  ...
  var API_CONFIGURED = !!API_BASE && API_BASE.indexOf('example.com') === -1 && !!PAGE_ID;
  ...
  function applyRemoteState(j) {
    ...
    if (Array.isArray(j.campaigns)) {
      ...
      /* удаляем строки, которых бол��ше нет в ответе API */
  ...
  function startSse() {
    ...
    try { sse = new EventSource(url); } catch (e) { startPolling(); return; }
    sse.onopen = function () {
      liveMode = 'sse';
      stopPolling();
    ...
    function onStateEvent(ev) {
      ...
      try {
        ...
        lastOk = true;
      } catch (e) { /* некорректный JSON — игнорируем */ }
    }
  ...
  try { window.__CHARTER_INIT__ = init; } catch (e) { /* noop */ }
