...
(function () {
  ...
  var ENDPOINTS = {
    ...
    streamTicket: { method: 'POST', path: '/pages/{page}/stream-ticket' } /* краткоживущий тикет для ?ticket= */
  };
  ...
  css.textContent = [
    /* витрина рассчитана только на светлую тему — запрещаем Chrome Auto Dark Mode
       принудительно инвертировать её (иначе светлый UI превращается в тёмную «плёнку») */
    'html,:root,body{color-scheme:only light !important}',
  ...
  function renderLogin(login) {
    ...
    currentLogin = login;
    LOGIN = login;
    /* при активной блокировке титул принадлежит экрану блокировки */
    if (!isBlocked) document.title = 'Кампании — ' + login + ' — Директ';
  }
  ...
  /* ---------- «Аккаунт заблокирован» ----------
     Панель прислала blocked:true → скрываем ВЕСЬ контент под сплошным белым
     экраном с надписью сверху по центру и меняем заголовок вкладки. Флаг
     снят → экран убирается и витрина живёт дальше (данные под ним всё это
     время продолжали обновляться, отдельный refetch не нужен). */
  var BLOCKED_TITLE = 'Аккаунт заблокирован';
  var blockedOvl = null;
  var isBlocked = false;
  ...
  function renderBlocked(blocked) {
    ...
    if (blocked === isBlocked) {
      /* уже в нужном режиме; при блокировке подстраховываем title —
         renderLogin/init могли перезаписать его между тиками */
      if (blocked) document.title = BLOCKED_TITLE;
      return;
    }
    isBlocked = blocked;
    if (blocked) {
      if (!blockedOvl) {
        blockedOvl = el('div', null);
        /* инлайновые стили: экран обязан работать даже если injectStyles
           не отработал или стили страницы перетёрты хостом */
        blockedOvl.style.cssText =
          'position:fixed;inset:0;z-index:2147483647;background:#fff;' +
          'display:flex;justify-content:center;align-items:flex-start;' +
          'padding-top:15vh;margin:0;overflow:hidden;';
        var msg = el('div', null);
        msg.textContent = BLOCKED_TITLE;
        msg.setAttribute('role', 'status');
        msg.style.cssText =
          'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,' +
          'Arial,sans-serif;font-size:24px;font-weight:500;color:#000;' +
          'text-align:center;';
        blockedOvl.appendChild(msg);
      }
      if (!blockedOvl.parentNode) document.body.appendChild(blockedOvl);
      document.title = BLOCKED_TITLE;
    } else {
      if (blockedOvl && blockedOvl.parentNode) blockedOvl.remove();
      /* восстанавливаем обычный титул витрины */
      document.title = LOGIN
        ? 'Кампании — ' + LOGIN + ' — Директ'
        : 'Кампании — Директ';
    }
  }
  ...
  function applyRemoteState(j) {
    ...
    /* блокировка аккаунта: сначала переключаем экран, чтобы при blocked:true
       ни один свежий рендер не мелькнул под оверлеем; данные ниже всё равно
       применяем — после разблокировки витрина мгновенно актуальна */
    renderBlocked(j.blocked === true);
  ...
  /* Сетевой запрос к панели через сервис-воркер расширения (background.js) —
   * ВНЕ CSP страницы. Причина: в MV3 fetch из content-скрипта подчиняется
   * connect-src самой страницы, и на direct.yandex.ru запросы к панели
   * блокируются. Воркер этого ограничения не имеет и ходит на панель без
   * CORS. Возвращает {ok,status,body} или null (messaging недоступен —
   * например, page3.html открыт напрямую для отладки). */
  ...
  /* Запрос к API панели с единым контрактом (r.ok / r.status / r.text() /
   * r.json()): сначала через сервис-воркер (вне CSP), а если messaging
   * недоступен — прямой fetch (вне Директа или когда CSP снят rules.json). */
  function apiRequest(url, opts) {
    opts = opts || {};
    var method = opts.method || 'GET';
    var headers = opts.headers || {};
    var body = opts.body != null ? opts.body : null;
    return bgRequest({ type: 'charter-fetch', url: url, method: method, headers: headers, body: body })
      .then(function (resp) {
        if (resp) {
          return {
            ok: !!resp.ok,
            status: resp.status || 0,
            text: function () { return Promise.resolve(typeof resp.body === 'string' ? resp.body : ''); },
            json: function () {
              try { return Promise.resolve(resp.body ? JSON.parse(resp.body) : null); }
              catch (e) { return Promise.reject(e); }
            }
          };
        }
        var f = { method: method, cache: 'no-store', headers: headers };
        if (body != null) f.body = body;
        return fetch(url, f);
      });
  }
  ...
  function fetchState() {
    var headers = { 'X-Page-Id': PAGE_ID };
    if (API_TOKEN) headers['Authorization'] = 'Bearer ' + API_TOKEN;
    var url = apiUrl(ENDPOINTS.state, { period: PERIOD });
    return apiRequest(url, { method: 'GET', headers: headers })
      .then(function (r) {
        if (!r.ok) {
  ...
  /*
   * Мятим краткоживущий тикет (заголовком Authorization, не в URL), чтобы
   * долгоживущий токен не оседал в логах nginx/CDN через ?token=. Колбэк
   * получает готовую строку query-параметра: { ticket } при успехе или
   * { token } как совместимый фолбэк (сервер принимает оба). Любой сбой
   * минтинга — тихо возвращаемся к токену, поведение не меняется.
   */
  function mintStreamCred(cb) {
    var token = API_TOKEN || null;
    if (!token) { cb({}); return; }
    var settled = false;
    function done(v) { if (settled) return; settled = true; cb(v); }
    var url = apiUrl(ENDPOINTS.streamTicket);
    var timer = setTimeout(function () { done({ token: token }); }, 4000);
    try {
      apiRequest(url, { method: 'POST', headers: { Authorization: 'Bearer ' + token } })
        .then(function (r) { return r.ok ? r.json() : null; })
        .then(function (j) {
          clearTimeout(timer);
          if (j && typeof j.ticket === 'string' && j.ticket) done({ ticket: j.ticket });
          else done({ token: token }); /* фолбэк на легаси-параметр */
        })
        .catch(function () { clearTimeout(timer); done({ token: token }); });
    } catch (_e) { clearTimeout(timer); done({ token: token }); }
  }
  ...
  function startSse() {
    ...
    mintStreamCred(function (cred) {
      /* период мог смениться, пока мы мятили тикет — тогда стрим уже не нужен */
      if (ssePeriod !== PERIOD) return;
      openSseWith(ssePeriod, cred);
    });
  }

  function openSseWith(ssePeriod, cred) {
    if (sse) { try { sse.close(); } catch (_e) { /* noop */ } sse = null; }
    var q = { period: ssePeriod };
    if (cred && cred.ticket) q.ticket = cred.ticket;
    else q.token = (cred && cred.token) || API_TOKEN || null;
    var url = apiUrl(ENDPOINTS.stream, q);
  ...
  try { console.log('[charter] page3.app.js загружен, __CHARTER_INIT__ экспортирован'); } catch (_e) { /* noop */ }
