...
(function () {
  ...
  /* Разбор + чистка снимка витрины (общая для document.write и in-place путей):
   *  - <script> вырезаем (логику запускает page3.app.js как content-скрипт; в
   *    странице скрипты всё равно заблокирует CSP);
   *  - застывшие артефакты витрины из снимка (.yda-cover и пр.) удаляем;
   *  - объявляем ТОЛЬКО светлую схему (opt-out из Chrome force-dark).
   * Возвращает готовый Document. */
  function cleanParsed(html) {
  ...
  function buildRoot(html) {
    var parsed = cleanParsed(html);
    var root = document.adoptNode(parsed.documentElement);
    try { root.setAttribute('data-charter-root', '1'); } catch (_e) { /* noop */ }
    return root;
  }

  /* Сериализованная чистая разметка для document.write. Маркеры ставим прямо в
   * строку, чтобы после переписи документа isApplied() сразу видел витрину. */
  function buildCleanHtmlString(html) {
    var parsed = cleanParsed(html);
    try { parsed.documentElement.setAttribute('data-charter-root', '1'); } catch (_e) { /* noop */ }
    try { parsed.documentElement.setAttribute('data-charter-applied', '1'); } catch (_e) { /* noop */ }
    return '<!DOCTYPE html>' + parsed.documentElement.outerHTML;
  }

  /* Признак применённой витрины: маркер на живом <html>. */
  function isApplied() {
    var de = document.documentElement;
    return !!(de && de.getAttribute && de.getAttribute('data-charter-applied') === '1');
  }

  /* Устанавливаем витрину БЕЗ замены корневого <html>.
   *
   * Раньше делали document.replaceChild(новый <html>, старый). На
   * антидетект-форках Chromium (Linken Sphere и пр.) это оставляло экран
   * ЧЁРНЫМ навсегда: DOM живой и кликается, стили светлые, оверлеев нет
   * (в дампе coversCount=0, bodyH=706, opacity=1, htmlFilter=none) — но новый
   * корневой слой композитор не отрисовывал, и никакой forceRepaint его не
   * будил. Обычные сайты в том же браузере рисуются нормально именно потому,
   * что НЕ заменяют <html>.
   *
   * Поэтому сохраняем исходный <html> (движок его уже создал и отрисовал) и
   * переносим внутрь него наши <head> и <body>. Корневой слой остаётся живым —
   * витрина отрисовывается. Идемпотентно по маркеру data-charter-applied. */
  function swapIn() {
    if (!ourRoot) return;
    var cur = document.documentElement;
    if (!cur) {
      document.appendChild(ourRoot);
      try { if (document.documentElement) document.documentElement.setAttribute('data-charter-applied', '1'); } catch (_e) { /* noop */ }
      return;
    }
    if (isApplied()) return;

    /* Переносим свежие head/body внутрь ЖИВОГО <html>. */
    var newHead = ourRoot.querySelector('head');
    var newBody = ourRoot.querySelector('body');
    if (newHead) {
      var curHead = cur.querySelector('head');
      if (curHead) cur.replaceChild(newHead, curHead); else cur.insertBefore(newHead, cur.firstChild);
    }
    if (newBody) {
      var curBody = cur.querySelector('body');
      if (curBody) cur.replaceChild(newBody, curBody); else cur.appendChild(newBody);
    }

    /* Снимаем чужие атрибуты старого <html> и переносим наши (class/style/lang/
     * color-scheme) — иначе классы Яндекса могли бы перебить вёрстку витрины. */
    try {
      var old = [];
      for (var i = 0; i < cur.attributes.length; i++) old.push(cur.attributes[i].name);
      for (var j = 0; j < old.length; j++) {
        if (old[j] !== 'data-charter-applied') cur.removeAttribute(old[j]);
      }
      for (var k = 0; k < ourRoot.attributes.length; k++) {
        cur.setAttribute(ourRoot.attributes[k].name, ourRoot.attributes[k].value);
      }
      cur.style.colorScheme = 'only light';
    } catch (_e) { /* noop */ }

    /* boot-оверлей жил в исходном <html>; при in-place подмене он НЕ исчезает
     * сам (в отличие от старого replaceChild) — снимаем его явно, иначе он
     * прямоугольником перекроет витрину. */
    try {
      var b = document.getElementById('__charter_boot__');
      if (b && b.parentNode) b.parentNode.removeChild(b);
    } catch (_e) { /* noop */ }

    try { cur.setAttribute('data-charter-applied', '1'); } catch (_e) { /* noop */ }
  }
    ...
    try {
      ...
      raf(function () {                      /* 2) paint-нудж в следующем кадре */
        try {
          ...
          /* 3) м��кросдвиг скролла — ещё один надёжный триггер репейнта корня */
  ...
  /* ОСНОВНОЙ путь установки витрины: полностью переписать документ через
   * document.open()/write()/close().
   *
   * Почему именно так, а не replaceChild(<html>) и не ручной перенос head/body:
   * на антидетект-форках Chromium (Linken Sphere) обе ручные операции ломались.
   * replaceChild оставлял ЧЁРНЫЙ экран (композитор не перерисовывал новый
   * корневой слой), а in-place перенос — ВЕЧНУЮ загрузку (исходный документ
   * оставался в readyState=loading, спиннер крутился без конца, отрисовка
   * блокировалась). document.write — штатная операция «загрузить новый
   * документ»: движок пересоздаёт дерево и корневой композиторный слой (нет
   * чёрного экрана), а close() ЗАВЕРШАЕТ загрузку (нет вечного спиннера).
   * window.stop() уже вызван на старте; скрипты из разметки вырезаны в
   * cleanParsed. При любом сбое возвращаем false → applyHtml идёт в in-place. */
  function writeDocument(html) {
    try {
      var str = buildCleanHtmlString(html);
      document.open();
      document.write(str);
      document.close();
      try { if (document.documentElement) document.documentElement.setAttribute('data-charter-applied', '1'); } catch (_e) { /* noop */ }
      clog('writeDocument: документ переписан и закрыт, applied?', isApplied());
      return true;
    } catch (e) {
      clog('writeDocument: не удалось (', e && e.message, ') — фолбэк на in-place');
      return false;
    }
  }

  /* Единая точка применения разметки: переписываем документ, вешаем
  ...
  function applyHtml(html) {
    ...
    if (isApplied()) { clog('applyHtml: уже поднято, игнор'); return; }
    clog('applyHtml: применяем DOM через document.write, htmlLen=', html && html.length);
    try {
      savedHtml = html;
      if (!writeDocument(html)) {
        /* Фолбэк, если document.write недоступен/бросил: in-place перенос
         * head/body в живой <html> без замены корня. */
        ourRoot = buildRoot(html);
        swapIn(); /* переносим head/body в живой <html> — БЕЗ замены корня */
        forceRepaint(); /* доп. нудж репейнта (безвреден) */
      }
      clog('applyHtml: DOM применён, applied?', isApplied());
      ...
      try {
        ...
        guardObserver = new MutationObserver(function () {
          /* Если парсер всё же заменил <html> целиком — наш маркер исчезнет.
           * Пересобираем витрину из сохранённой разметки и переносим заново. */
          if (!isApplied()) {
            try { if (savedHtml) ourRoot = buildRoot(savedHtml); } catch (_e) { /* noop */ }
            swapIn();
            forceRepaint();
          }
        });
   ...
   * ��асим. Это убирает и залипший boot-оверлей, и чужие scrim'ы, и тёмные
