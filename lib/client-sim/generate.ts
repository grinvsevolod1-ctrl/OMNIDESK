...
function askedQuestionsBlock(ownLines: string[]): string {
  ...
  return [
    '',
    'ВОПРОСЫ, КОТОРЫЕ ТЫ УЖЕ ЗАДАВАЛ (НЕ задавай их снова — ни этими словами, ни ��ругими, это ОДИН И ТОТ ЖЕ вопрос):',
    list,
...
function timeBlock(): string {
  ...
  const partOfDay =
    h < 5 ? 'глубокая ночь' : h < 11 ? 'утро' : h < 17 ? 'ден��' : h < 23 ? 'вечер' : 'ночь'
...
/**
 * True when a line is the canonical web-form (platform notification) opening.
 * The engine must post it as ONE message: a real site notification arrives as
 * a single block, so bursting it into chat-style bubbles reads as fake.
 * The prefix is constant regardless of the configurable site name.
 */
export function isWebFormOpening(body: string): boolean {
  return body.startsWith('Здравствуйте! Я прошёл ИИ-подбор на сайте')
}
