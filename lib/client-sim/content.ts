...
const FILLERS = [
  'ало', 'вы тут', 'ну что там', 'че молчите', 'долго ещё ждать', 'эй', 'есть кто',
  'жду ответ', 'ну', 'и', 'а дальше', 'че как',
]
...
export function templateLine(kind: TemplateKind, persona: SimPersona): string {
  ...
  const base = fill(pick(pool), persona)
  // Occasionally staple two short fragments together for variety.
  const combined =
    kind !== 'opener' && chance(0.25)
      ? `${base} ${fill(pick(kind === 'angry' ? ANGRY : CURIOUS), persona)}`
      : base
  return applyStyle(combined, persona.style)
}
