...
export function curatorTools(_state: RunState) {
  return {
    listCurators: tool({
      description:
        'Показать всех активных менеджеров по кадрам: имя, города (менеджер по кадрам может вести несколько городов) и текущая нагрузка (сколько активных лидов у каждого). Вызывай, когда админ спрашивает «какие у нас менеджеры по кадрам», «кто ведёт Казань», «кто самый загруженный», «кому можно отдать лида».',
    ...
    curatorDiscipline: tool({
      description:
        'Сводка дисциплины менеджеров по кадрам: сегодня (сколько лидов, сколько статусов подтверждено, сколько просрочено, распределение по статусам) плюс история за 30 дней — процент подтверждений, сделанных вовремя (до 10:00 МСК). Вызывай, когда админ спрашивает «как дела у менеджеров по кадрам», «кто не обновил статусы», «как дисциплина», «кто стабильно опаздывает».',
      inputSchema: z.object({}),
      execute: async () => {
    ...
    curatorLeads: tool({
      description:
        'Лиды конкретного менеджера по кадрам (по имени) или все переданные лиды с фильтрами по статусу и городу — включая «осиротевшие» лиды без менеджера по кадрам. Вызывай, когда админ спрашивает «покажи лиды Марии», «сколько лидов в работе по Москве», «есть ли лиды без менеджера по кадрам».',
      inputSchema: z.object({
        curatorName: z
          .string()
          .optional()
          .describe('Имя менеджера по кадрам (частичное совпадение), если спрашивают про конкретного'),
        status: z
          .string()
          .optional()
          .describe(
        ...
        orphanedOnly: z
          .boolean()
          .optional()
          .describe('true — только лиды без менеджера по кадрам (осиротевшие)'),
      }),
      execute: async ({ curatorName, status, city, orphanedOnly }) => {
        ...
        if (curatorName?.trim()) {
          ...
          if (!found) {
            return {
              ok: false,
              error: `Менеджер по кадрам с именем «${curatorName}» не найден. Доступны: ${all.map((c) => c.name).join(', ') || 'никого'}.`,
            }
          }
          curatorId = found.id
