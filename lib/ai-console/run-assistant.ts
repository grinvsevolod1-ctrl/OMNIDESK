...
export async function prepareAssistantRun(
  ...
  const tools = {
    ...
    rememberDirective: tool({
      description:
        'ЗАПОМНИТЬ ПРЯМОЕ УКАЗАНИЕ (правило/сценарий) для ИИ-менеджера — то, что админ диктует своими словами: «всегда сначала спрашивай бюджет», «никогда не обещай скидку ��ольше 10%», «если упомянут конкурент — делай так». Правило durable (не стирается обучением) и попадает в КАЖДЫЙ ответ клиенту во всех каналах, включая Telegram, с высшим приоритетом. Вызывай, когда админ говорит «запомни», «правило», «всегда/никогда», «делай так», «вот сценарий». Можешь слегка причесать формулировку, не меняя смысла.',
      inputSchema: z.object({
    ...
    getPerformance: tool({
      description:
        'Свести реальные результаты работы ИИ-менеджера за период: сколько диалогов, сколько ликвидных лидов, сколько передано человеку, конверсия. Вызывай, когда админ спрашивает «как дела за неделю», «сколько дожали», «где теряем клиентов», «какая конверсия». Данные ТОЛЬКО по реальным клиентам. Передай days (по умолчанию 7).',
      inputSchema: z.object({
