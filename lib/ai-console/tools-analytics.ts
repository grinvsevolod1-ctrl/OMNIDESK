...
export function analyticsTools(state: RunState) {
  return {
    ...
    analyzeLosses: tool({
      description:
        'Пакетный разбор ПРОИГРЫШЕЙ: прочитать слитые диалоги за период (клиент ушёл, не ликвид, передан человеку), сгруппировать причины по кластерам с долями («40% погибло на возражении по цене, 25% — долго не отвечали») �� получить конкретное контр-предложение по каждому кластеру. Вызывай, когда админ спрашивает «где мы теряем клиентов», «почему сливаются», «разбери проигрыши за месяц». Это глубже findWeakSpots (тот даёт точечные уроки): здесь — карта утечек с приоритетами. Доложи кластеры от большего к меньшему и предложи закрыть самый крупный первым; правила/уроки сохраняй только с согласия админа (rememberDirective/addLesson).',
      inputSchema: z.object({
        days: z.number().int().min(1).max(180).optional(),
