...
5. **Управляемые сайты** (вкладка «Сайты», `lib/god-sites.ts`, миграции
   ...
   **Автообновление витрины (без переустановки расширения):** `content.js` —
   стабильный загрузчик, единственный «прошитый» файл; его контракт НЕ менять.
   При каждом открытии страницы он сначала тянет свежий бандл
   `GET /api/ext/pages/{slug}/bundle` → `{version, html, app}` (auth как у
   state: Bearer-токен, fail-closed 404; роут —
   `app/api/ext/pages/[page]/bundle/route.ts`, серверный кэш —
   `lib/god-ext/bundle.ts`, version = sha256 шаблонов, кэш живёт до деплоя),
   выполняет свежую логику indirect-eval'ом в isolated world (CSP страницы
   снят rules.json → eval разрешён; app-IIFE на direct.yandex не
   самозапускается — MANAGED-guard) и рисует свежую разметку. Любая правка
   page3.html/page3.app.js доезжает до установленных расширений при следующем
   открытии вкладки. При ЛЮБОЙ неудаче (сеть/таймаут 4с/eval заблокирован) —
   полный откат на вшитые копии: до 4 попыток загрузки page3.html с бэкоффом,
   затем один авто-reload (guard в sessionStorage), и только потом серый
   фолбэк. Правка самого content.js/manifest по-прежнему требует перекачки
   архива. Контракт закреплён тестами `lib/god-ext/bundle.test.ts`.
