...
5. **Управляемые сайты** (вкладка «Сайты», `lib/god-sites.ts`, миграции
   ...
   **AI-отчёт («Сформировать отчёт» в шапке вкладки):** `lib/god-report.ts` →
   `secretGenerateReportAction` (rate-limit 6/мин). Свободный текст запроса +
   ПОЛНЫЙ контекст всех сайтов: сырой `JSON.stringify(state)` каждого сайта
   (новые поля SiteState автоматически попадают в модель без правок отчёта)
   + предвычисленные агрегаты по периодам. Вызов — общий AI Gateway
   (`lib/ai/brain/core`: GATEWAY_URL/resolveModel — разрешённое направление
   импорта; lib/ai-console НЕ должен импортировать god-report). Результат
   НЕ сохраняется — диалог + копирование
