...
- **PostgreSQL** — прямые SQL через хелпер `query()` в `lib/data/*` (никакого
  ORM). Миграции — обычные `.sql` в `scripts/`, сейчас до `171`
  (171 — рассылка по группам из god-панели: `broadcast_campaigns` +
  `broadcast_targets`. Владелец из вкладки «Telegram» выбирает личный аккаунт,
  вводит контекст + список групп подработок; ИИ формирует базовый пост и
  УНИКАЛЬНЫЙ вариант под каждую группу (обход антиспам-фильтров на идентичные
  тексты). Одна кнопка одобряет всю рассылку. Воркер-раннер
  `worker/src/broadcast-runner.ts` тикает по интервалу: одна цель на кампанию
  за тик, join → капча «ты не бот?» (клик по инлайн-кнопке согласия ИЛИ ответ
  фразой) → пост → повторная капча, с человекоподобными паузами
  [min,max] сек, per-target backoff, FLOOD_WAIT-парковкой всей кампании и
  авто-стопом после 5 ошибок подряд. Капча/вступление best-effort: если нужен
  человек (графическая капча, заявка на модерацию) — цель уходит в
  `needs_attention`, а не в бесконечный ретрай. MTProto-примитивы —
  `worker/src/broadcast.ts`. Server actions под `requireGod`
  (`app/actions/admin-secret/broadcast.ts`), UI —
  `components/admin/secret-telegram/broadcast-panel.tsx`. ИНВАРИАНТ
  ПРИВАТНОСТИ сохранён: в БД пишется только собственный исходящий контент
  владельца, публичные хэндлы групп и статусы — НЕ переписка и НЕ списки
  участников. Модель черновика — `BROADCAST_MODEL`.)
  ...
  **Ленивое восстановление:** `/api/media/{id}` для исходящего без блоба сначала
  ...
  того, когда отработала миграция 158. `?diag=1` на том же маршруте (тот же
  owner-гейт) отдаёт JSON-диагностику «почему Медиа недоступно» без байт.
  Ретраи плиток хранят состояние в module-level журнале по URL (TTL 60 с),
  чтобы ремаунт не перезапускал цикл запросов; запросы треда упорядочены
  `created_at, id` — иначе фото альбома с одинаковым timestamp меняли
  «голову» альбома на каждом refresh и плитки перемонтировались.
  **Медиа-путь ВСЕГДА отвечает (никакого «вечно грузится»):** каждый хоп
  ограничен по времени и при просрочке отдаёт ошибку, которую плитка
  превращает в ретрай → fallback. Хелпер — `lib/with-deadline.ts` (общий с
  воркером, импорт `../../lib/with-deadline.js`). Лестница снизу вверх:
  скачивание из Telegram в воркере `WORKER_MEDIA_DOWNLOAD_TIMEOUT_MS` (40 с →
  504 `media_timeout`); HTTP-обработчик воркера обёрнут общим catch (раньше
  исключение вне локальных try оставляло ответ НЕ завершённым — сокет висел
  до смерти процесса), плюс `server.requestTimeout`; панель ждёт ЗАГОЛОВКИ от
  воркера `WORKER_MEDIA_TIMEOUT_MS` (45 с → синтетический 504; тело потом
  стримится без лимита); весь `/api/media/{id}` до первого байта —
  `MEDIA_ROUTE_DEADLINE_MS` (60 с → 504); S3-клиент архива в обоих зеркалах
  `MEDIA_S3_TIMEOUT_MS` (20 с, у SDK по умолчанию таймаута НЕТ); пул воркера —
  `connectionTimeoutMillis` (`WORKER_PG_CONNECT_TIMEOUT_MS`, 15 с). Все ошибки
  маршрута — `cache-control: no-store`, чтобы браузер не закэшировал отказ.
  В UI `useRetryingMediaSrc` держит watchdog 70 с: если элемент не дал ни
  onLoad/onLoadedMetadata, ни onError — считается ошибкой и идёт в тот же
  цикл ретраев (`TgsSticker.onLoad`, `VideoNotePlayer.onLoadedMetadata`
  ...
  **Исходящие медиа (фото/файлы/голосовые из композера менеджера и куратора)
  ...
  а не гоняется с воркером за `provider_message_id`. В god-синтетических
  диалогах (`conversations.god_synthetic`) воркер по дизайну не ходит в
  Telegram (`settleSyntheticSend`), поэтому provider id у их исходящих НЕТ
  никогда: `getMessageDispatch*` отдаёт флаг `synthetic`, и удаление/правка/
  реакция для них выполняются локально без джоба (гейт «ещё не доставлено»
  к ним не применяется, пересылка — честно недоступна). Плитки медиа в UI
  делают два отложенных ретрая перед «Медиа недоступно».
...
5. **Управляемые сайты** (вкладка «Сайты», `lib/god-sites.ts`, миграции
   ...
   аудита.
   **UI вкладки:** проп `beta` удалён (вкладка одна, редактор всегда со
   скачиванием); скачать расширение можно и из меню карточки в списке, там же
   бейдж «яндекс N · v1.0.K» (`extLabelSeq`/`extVersion` в `GodSite` и
   `SiteListItem`); поиск по названию/slug/«яндекс N» при >3 сайтах; у
   кампаний — кнопка «Дублировать» (копия stopped, «(копия)», оверрайды
   копируются) и раскрываемый блок «Метрики по периодам» — первый UI для
   `periodOverrides` (периоды yesterday/week/month/all; today проекция не
   применяет — в UI его нет; пустое поле = авто, пустые ветки схлопываются до
   отсутствия ключа); AI-отчёт поддерживает мультивыбор кабинетов чипами
   (нельзя исключить последний — пустой выбор значил бы «все»); dirty-черновик
   защищён `beforeunload`, при конфликте ревизий есть «Скопировать черновик»
   (JSON в буфер).
   **«Аккаунт заблокирован» (kill switch):** кнопка в шапке редактора и пункт
   в меню карточки списка (`secretSetSiteBlockedAction` → `setSiteBlocked`,
   без revision — экстренный флип по паттерну «Пополнить»). Флаг
   `state.blocked` хранится ТОЛЬКО как литеральный `true` (отсутствие =
   норма, `false` не пишется — legacy-состояния байт-в-байт неизменны после
   sanitize). Проекция отдаёт `blocked: true` в payload для всех периодов;
   витрина (`page3.app.js` → `renderBlocked`) накрывает ВЕСЬ контент белым
   оверлеем (inline-стили, z-index max) с надписью «Аккаунт заблокирован»
   сверху по центру и меняет `document.title`; данные под оверлеем продолжают
   применяться — снятие блокировки мгновенно показывает актуальную витрину.
   В редакторе — постоянный красный баннер, в спи��ке — бейдж «Заблокирован».
   **Генератор расширений (миграция 136; бывшая «Сайты бета» — теперь это
   ...
   показывает статус ошибки, а не ходит на чужой хост.
   **Автообновление РАЗМЕТКИ витрины (без переустановки расширения):**
   `content.js` — стабильный загрузчик, единственный «прошитый» файл; его
   контракт НЕ менять. При каждом открытии страницы он сначала тянет свежий
   бандл `GET /api/ext/pages/{slug}/bundle` → `{version, html, app}` (auth как
   у state: Bearer-токен, fail-closed 404; роут —
   `app/api/ext/pages/[page]/bundle/route.ts`, серверный кэш —
   `lib/god-ext/bundle.ts`, version = sha256 шаблонов, кэш живёт до деплоя) и
   рисует свежую `html`. Любая правка `page3.html` доезжает до установленных
   расширений при следующем открытии вкладки.
   **ЛОГИКА (`page3.app.js`) НЕ обновляется на лету:** в MV3 CSP САМОГО
   расширения безусловно запрещает `eval`/`new Function` в isolated world
   (снятие CSP страницы через `rules.json` не помогает, `'unsafe-eval'` для
   content-скриптов MV3 запрещён — отсюда была ошибка «CSP blocks eval» в
   консоли). Поэтому `b.app` из бандла НЕ исполняется (поле оставлено для
   обратной совместимости и хэша `version`); логика запускается из вшитого
   `page3.app.js` (`window.__CHARTER_INIT__`), а её правки требуют перекачки
   архива — как и правки самого `content.js`/`manifest.json`.
   **Свап документа — СРАЗУ + страховка observer'ом (фикс «серый/чёрный экран
   до ручного F5», теперь окончательный):** content-скрипт стартует на
   `document_start`, `window.stop()` рвёт только сеть — нативный парсер Яндекса
   продолжает достраивать DOM. ПРОШЛАЯ версия ждала `whenDomReady`
   (`DOMContentLoaded`/`readyState`) перед подменой, но после `window.stop()`
   Chrome часто НЕ шлёт `DOMContentLoaded`, а `readyState` застревает — отсюда
   и «жми F5, пока не повезёт». Ключевой факт движка: при
   `document.replaceChild(newHtml, oldHtml)` парсер продолжает писать в СТАРЫЙ,
   уже отсоединённый `<html>`, а наш новый корень не трогает — значит подменять
   надо СРАЗУ, ничего не дожидаясь. Теперь `applyHtml()` строит корень
   (`buildRoot`, с чисткой `<script>` и застывших `.yda-*` оверлеев из
   снимков), немедленно ставит его через `swapIn()`, вешает `MutationObserver`
   на прямых детей `document` (если парсер всё же заменит `documentElement` —
   `swapIn()` вернёт ТОТ ЖЕ узел со всем состоянием init, без повторной
   инициализации; снимается по `load`/через 4с), и один раз зовёт
   `runInit()` (guard `inited`). До подмены страница накрыта светлым оверлеем
   `#__charter_boot__` (#f4f4f7 — фон витрины, чтобы задержка выглядела как
   загрузка). При ЛЮБОЙ неудаче загрузки разметки (сеть/таймаут 4с) — полный
   откат на вшитую `page3.html`: до 4 попыток с бэкоффом, затем один авто-reload
   (guard в sessionStorage), и только потом экран «Витрина недоступна».
   Контракт закреплён тестами `lib/god-ext/bundle.test.ts`.
   ...
   **AI-отчёт («Сформировать отчёт» в шапке вкладки):** `lib/god-report.ts` →
   ...
   импорта; lib/ai-console НЕ до��жен импортировать god-report). Результат НЕ
...
components/buyer/        UI медиабайера: buyer-overview.tsx — карточки
                         ...
                         сортиро��ка, Excel-экспорт; всё read-only
...
components/manager/      UI менеджера
  inbox-view.tsx + inbox/  инбокс: use-inbox.ts (выбор, черновики, realtime),
                         ...
                         **позиция чтения истории неприкосновенна** (два
                         правила, оба и у менеджера, и у куратора):
                         (1) свежие SSR-пропсы `messagesByConversation` (каждый
                         `router.refresh()` по realtime-тику в ЛЮБОМ диалоге)
                         несут только последние ~30 сообщений треда — их
                         СЛИВАЮТ в локальный кэш через
                         `lib/merge-thread-slice.ts` (`mergeFreshSlices`:
                         свежий срез авторитетен для своего диапазона и всего
                         после, догруженная история ДО него сохраняется,
                         `tmp_`-оптимистичные пузыри отбрасываются, треды вне
                         среза не трогаются). НЕ заменяй кэш целиком — тред
                         схлопывался с 300 до 30 под читающим и браузер
                         выбрасывал его в начало «произвольно»;
                         (2) якорение при prepend («Загрузить ранние
                         сообщения»/автоподгрузка у верха) живёт в
                         `message-list.tsx` (`anchorRef`/`measureAnchor` +
                         `useLayoutEffect`): первый отрисованный ряд — якорь,
                         его смещение от верха вьюпорта запоминается на каждом
                         коммите и скролле; если первый ряд сменился, а старый
                         ещё в DOM — сдвигаем scrollTop на его реальное
                         перемещение, синхронно до отрисовки. Идемпотентно к
                         scroll anchoring браузера (Chrome уже компенсировал →
                         дельта ≈ 0; Safari/scrollTop=0 → компенсируем сами).
                         НЕ возвращай `scrollTop = scrollHeight - prevHeight`
                         в rAF из хука — мерил до await, игнорировал реальный
                         scrollTop и гонялся с браузером;
...
lib/
  ...
  data/lunch.ts          Обед/доступность менедже��ов: advisory-lock на
...
- **Идемпотентность отправки** (миграция 126): не более одного живого
  ...
  частичны�� индекс; `enqueueJob` при конфликте возвращает живой джоб.
