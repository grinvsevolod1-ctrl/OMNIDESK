...
5. **Управляемые сайты** (вкладка «Сайты», `lib/god-sites.ts`, миграции
   ...
   **Локальный запуск, без онлайн-обновления разметки/логики:** сборщик
   генерирует `page3.markup.js` — HTML из пакета как JSON-экранированную строку
   `window.__CHARTER_HTML__`. Порядок content scripts: `config.js` →
   `page3.markup.js` → `page3.app.js` → `content.js` на `document_start`.
   Загрузчик останавливает исходную загрузку, очищает снимок от скриптов и
   застывших `.yda-*` артефактов, один раз переносит готовые `head/body`
   внутрь существующего `<html>` и запускает `__CHARTER_INIT__`.
   `document.open/write` НЕ используются: в проверке установленного расширения
   Chromium после `window.stop()` они оставляли пустой DOM. Запуск разметки
   НЕ ждёт API, fetch, service worker, DOMContentLoaded или таймеров.
   Данные продолжают приходить через существующий background-прокси; новые
   архивы выбирают `transport: 'poll'`, без параллельного SSE. CSS/шрифты/
   изображения исходного шаблона по-прежнему загружаются с CDN Яндекса — это
   не полностью офлайн-страница. Изменения разметки/логики требуют перекачать
   архив; постоянный токен и старые архивы не инвалидируются. `/bundle`
   сохранён для ранее установленных расширений; новый загрузчик его не
   использует. Никакого исполнения строк (`eval`/`new Function`).
   **Регрессия зависания:** `startCoverGuard` наблюдал за `style/class`, а
   `neutralizeCovers` вызывал `forceRepaint`, менявший эти же стили. Это
   зацикливало MutationObserver и блокировало paint/таймеры (воспроизведено
   в Chromium). Watchdog дополнительно проверял `ourRoot`, остававшийся
   `null` после document.write, и мог затереть страницу через 14 секунд.
   Обе системы удалены: нет наблюдателей загрузчика, сканирования DOM,
   forceRepaint и автоматического reload. Живые модалки и `blocked:true`
   не трогаем; ошибка старта показывает кнопку ручного повтора. Локальные
   файлы проверяются ДО `window.stop()`: иначе повреждённый архив оставлял
   экран ошибки без кадров и с некликабельной кнопкой. При такой ошибке
   ещё работающий исходный парсер завершается штатно; только аварийный
   экран ждёт его `DOMContentLoaded` одноразовым обработчиком. Успешный
   запуск не ждёт этого события.
   Разрешения: origin панели и два `direct.yandex`-хоста;
   `declarativeNetRequestWithHostAccess`, правила только для `main_frame`.
   Тесты: `lib/god-ext/{build,bundle}.test.ts`, `pnpm test:extension`
   (установленный ZIP в Chromium; страница и API — фикстуры; один визуальный
   сценарий со штатным CDN, остальные с отключёнными внешними ресурсами).
