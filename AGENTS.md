...
5. **Управляемые сайты** (вкладка «Сайты», `lib/god-sites.ts`, миграция 132) —
   ...
   **Генератор расширений (вкладка «Сайты бета», миграция 136):** та же
   ...
   (сквозная нумерация с 11, MAX+1, идемпотентно; гонка первых скачиваний
   ловится retry по 23505). ПОРЯДОК АТОМАРНОСТИ: zip собирается ПЕРВЫМ с
   candidate-ключом, ротация (`rotateSiteKey(id, presetKey)`) коммитится
   ПОСЛЕДНЕЙ — упавшая сборка оставляет старый ключ рабочим, сайт никогда не
   остаётся без единого валидного токена. В шаблоне `page3.app.js` НЕТ
   fallback-конфига (домена/токена): единственный источник —
   сгенерированный `config.js`; без него витрина показывает статус ошибки,
   а не ходит на чужой хост. `content.js` устойчив к перезапуску браузера:
   до 4 попыток загрузки page3.html с бэкоффом, затем один авто-reload
   (guard в sessionStorage), и только потом серый фолбэк. Классическая
   вкладка «Сайты» с ручным токеном НЕ трогается — beta живёт рядом.
