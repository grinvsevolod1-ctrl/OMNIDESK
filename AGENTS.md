...
5. **Управляемые сайты** (вкладка «Сайты», `lib/god-sites.ts`, миграции
   ...
   **Свап документа ждёт готовности парсера (фикс «серый экран до ручного
   F5»):** content-скрипт стартует на `document_start`, `window.stop()` рвёт
   только сеть — нативный парсер Яндекса продолжает достраивать DOM. Ранняя
   `document.replaceChild` затиралась парсером → серый экран. Теперь свап
   (`replaceDocument`) выполняется только из `render()` → `whenDomReady()`
   (после `readyState !== 'loading'` / `DOMContentLoaded`, страховочный
   таймаут 1500 мс); до этого частично распарсенная страница накрыта тёмным
   оверлеем `#__charter_boot__`, исчезающим вместе со старым documentElement.
   При ЛЮБОЙ неудаче загрузки разметки (сеть/таймаут 4с) — полный откат на
   вшитую `page3.html`: до 4 попыток с бэкоффом, затем один авто-reload
   (guard в sessionStorage), и только потом серый фолбэк. Контракт закреплён
   тестами `lib/god-ext/bundle.test.ts`.
