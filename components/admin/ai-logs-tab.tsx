        ...
        <div className="flex max-h-[34rem] flex-col divide-y divide-border/60 overflow-y-auto">
          {logs.length === 0 ? (
            <p className="py-16 text-center text-sm text-muted-foreground">
              Пока нет событий. Как только ИИ начнёт обрабатывать сообщения или
              симулятор — создавать диалоги, здесь появятся записи в реальном
              времени.
            </p>
          ) : (
...
function DiagnosticsBanner({ diag }: { diag: AiDiagnostics | null }) {
  ...
  if (diag.simEnabled && !diag.simEngineRunning)
    problems.push(
      'Симулятор включён, но движок не запущен в этом процессе — перезапустите панель или переключите тумблер.',
    )
  if (diag.simEnabled && diag.simUsableChannels === 0)
    problems.push(
      'Для симулятора нет подходящих каналов — выберите каналы в настройках симулятора.',
    )
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <p className="font-medium">
          {healthy
            ? 'Система в порядке — ИИ и симулятор настроены корректно.'
            : 'Обнаружены причины, по которым ИИ или симулятор могут молчать:'}
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        ...
        <StatusChip ok={diag.simEnabled} label="Симулятор" neutralWhenOff />
        <Badge variant="outline" className="gap-1">
          Активных диалогов симулятора: {diag.simActiveThreads}
        </Badge>
        <Badge variant="outline" className="gap-1">
          Каналов для симулятора: {diag.simUsableChannels}
        </Badge>
      </div>
    </Card>
  )
